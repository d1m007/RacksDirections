    ...
    public static function showItems(Rack $rack)
    {
       ...
       //fill rows
        ...
        foreach ($items as $row) {
			...
			// Modification to set rack direction according to administrator's selection:
			if(isset($_SESSION['glpi_js_toload']['rack']) && $_SESSION['glpi_js_toload']['rack'][0] == 'js/rack.reverse.js') $y_var = $row['position'] - 1;
			else $y_var = $rack->fields['number_units'] - $row['position'];

            $gs_item = [
                'id'        => $row['id'],
                'name'      => $item->getName(),
                'x'         => $row['hpos'] >= 2 ? 1 : 0,
                'y'         => $y_var,	// Modification to set the correct rack direction
                'height'    => 1,
                'width'     => 2,
                'bgcolor'   => $row['bgcolor'],
                'picture_f' => null,
            ...
            if ($model->getFromDB($item->fields[$modelsfield])) {
                if ($model->fields['required_units'] > 1) {
					...
					// Modification to set rack direction according to administrator's selection:
					if(isset($_SESSION['glpi_js_toload']['rack']) && $_SESSION['glpi_js_toload']['rack'][0] == 'js/rack.reverse.js') $gs_item['y'] = $row['position'] - 1;
					else {
